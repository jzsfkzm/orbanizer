 ...
 *  v2.0.3 released 2016-07-04 17:42
...
}]};try{var d=function(){"use strict";for(var a=document.getElementsByTagName("script"),b=0;b<a.length;b++)if(a[b].src.match(/(jquery\.facedetection(\.min)?\.js)|\/ccv\.js/))return a[b].src},e=d()}catch(a){}if(void 0===f){var f=function(a,b){"use strict";return f.core[b.toString()]=b().core,function(){var c,d,e,f;if(arguments.length>1)for(d=arguments[arguments.length-2],e=arguments[arguments.length-1],f=new Array(arguments.length-2),c=0;c<arguments.length-2;c++)f[c]=arguments[c];else d=arguments[0].async,e=arguments[0].worker,f=arguments[0],delete f.async,delete f.worker,f=[f];var g={shared:{}},h=b.apply(g,f);return d?function(d,i){var j=0,k=new Array(e),l=h.pre.apply(g,[e]);for(c in g.shared)"function"==typeof g.shared[c]?delete g.shared[c]:void 0!==g.shared[c].tagName&&delete g.shared[c];for(c=0;c<e;c++){var m=new Worker(a);m.onmessage=function(a){return function(b){k[a]="string"==typeof b.data?JSON.parse(b.data):b.data,j++,j==e&&d(h.post.apply(g,[k]))}}(c);var n={input:l[c],name:b.toString(),shared:g.shared,id:c,worker:f.worker_num,from:"jquery.facedetection"};try{m.postMessage(n)}catch(a){m.postMessage(JSON.stringify(n))}}}:h.post.apply(g,[[h.core.apply(g,[h.pre.apply(g,[1])[0],0,1])]])}};f.core={}}var g={pre:function(a){if("img"==a.tagName.toLowerCase()){var b=document.createElement("canvas");document.body.appendChild(a),b.width=a.offsetWidth,b.style.width=a.offsetWidth.toString()+"px",b.height=a.offsetHeight,b.style.height=a.offsetHeight.toString()+"px",document.body.removeChild(a);var c=b.getContext("2d");return c.drawImage(a,0,0),b}return a},grayscale:function(a){for(var b,c,d=a.getContext("2d"),e=d.getImageData(0,0,a.width,a.height),f=e.data,g=a.width*a.height*4;g>0;)f[g-=4]=f[b=g+1]=f[c=g+2]=.3*f[g]+.59*f[b]+.11*f[c];return d.putImageData(e,0,0),a},array_group:function(a,b){var c,d,e=new Array(a.length);for(c=0;c<a.length;c++)e[c]={parent:-1,element:a[c],rank:0};for(c=0;c<a.length;c++)if(e[c].element){for(var f=c;e[f].parent!=-1;)f=e[f].parent;for(d=0;d<a.length;d++)if(c!=d&&e[d].element&&b(e[c].element,e[d].element)){for(var g=d;e[g].parent!=-1;)g=e[g].parent;if(g!=f){e[f].rank>e[g].rank?e[g].parent=f:(e[f].parent=g,e[f].rank==e[g].rank&&e[g].rank++,f=g);for(var h,i=d;e[i].parent!=-1;)h=i,i=e[i].parent,e[h].parent=f;for(i=c;e[i].parent!=-1;)h=i,i=e[i].parent,e[h].parent=f}}}var j=new Array(a.length),k=0;for(c=0;c<a.length;c++){d=-1;var l=c;if(e[l].element){for(;e[l].parent!=-1;)l=e[l].parent;e[l].rank>=0&&(e[l].rank=~k++),d=~e[l].rank}j[c]=d}return{index:j,cat:k}},detect_objects:f(e,function(a,c,d,e){function f(a){var b=this.shared.canvas,c=this.shared.interval,d=this.shared.scale,e=this.shared.next,f=this.shared.scale_upto,g=new Array(4*(f+2*e)),h=new Array(4*(f+2*e));g[0]=b,h[0]={width:g[0].width,height:g[0].height,data:g[0].getContext("2d").getImageData(0,0,g[0].width,g[0].height).data};var i;for(i=1;i<=c;i++)g[4*i]=document.createElement("canvas"),g[4*i].width=Math.floor(g[0].width/Math.pow(d,i)),g[4*i].height=Math.floor(g[0].height/Math.pow(d,i)),g[4*i].getContext("2d").drawImage(g[0],0,0,g[0].width,g[0].height,0,0,g[4*i].width,g[4*i].height),h[4*i]={width:g[4*i].width,height:g[4*i].height,data:g[4*i].getContext("2d").getImageData(0,0,g[4*i].width,g[4*i].height).data};for(i=e;i<f+2*e;i++)g[4*i]=document.createElement("canvas"),g[4*i].width=Math.floor(g[4*i-4*e].width/2),g[4*i].height=Math.floor(g[4*i-4*e].height/2),g[4*i].getContext("2d").drawImage(g[4*i-4*e],0,0,g[4*i-4*e].width,g[4*i-4*e].height,0,0,g[4*i].width,g[4*i].height),h[4*i]={width:g[4*i].width,height:g[4*i].height,data:g[4*i].getContext("2d").getImageData(0,0,g[4*i].width,g[4*i].height).data};for(i=2*e;i<f+2*e;i++)g[4*i+1]=document.createElement("canvas"),g[4*i+1].width=Math.floor(g[4*i-4*e].width/2),g[4*i+1].height=Math.floor(g[4*i-4*e].height/2),g[4*i+1].getContext("2d").drawImage(g[4*i-4*e],1,0,g[4*i-4*e].width-1,g[4*i-4*e].height,0,0,g[4*i+1].width-2,g[4*i+1].height),h[4*i+1]={width:g[4*i+1].width,height:g[4*i+1].height,data:g[4*i+1].getContext("2d").getImageData(0,0,g[4*i+1].width,g[4*i+1].height).data},g[4*i+2]=document.createElement("canvas"),g[4*i+2].width=Math.floor(g[4*i-4*e].width/2),g[4*i+2].height=Math.floor(g[4*i-4*e].height/2),g[4*i+2].getContext("2d").drawImage(g[4*i-4*e],0,1,g[4*i-4*e].width,g[4*i-4*e].height-1,0,0,g[4*i+2].width,g[4*i+2].height-2),h[4*i+2]={width:g[4*i+2].width,height:g[4*i+2].height,data:g[4*i+2].getContext("2d").getImageData(0,0,g[4*i+2].width,g[4*i+2].height).data},g[4*i+3]=document.createElement("canvas"),g[4*i+3].width=Math.floor(g[4*i-4*e].width/2),g[4*i+3].height=Math.floor(g[4*i-4*e].height/2),g[4*i+3].getContext("2d").drawImage(g[4*i-4*e],1,1,g[4*i-4*e].width-1,g[4*i-4*e].height-1,0,0,g[4*i+3].width-2,g[4*i+3].height-2),h[4*i+3]={width:g[4*i+3].width,height:g[4*i+3].height,data:g[4*i+3].getContext("2d").getImageData(0,0,g[4*i+3].width,g[4*i+3].height).data};return[h]}function h(a,b,c){var d,e,f,g,h,i,j=this.shared.cascade,k=(this.shared.interval,this.shared.scale),l=this.shared.next,m=this.shared.scale_upto,n=1,o=1,p=[0,1,0,1],q=[0,0,1,1],r=[];for(d=0;d<m;d++){var s=a[4*d+8*l].width-Math.floor(j.width/4),t=a[4*d+8*l].height-Math.floor(j.height/4),u=[4*a[4*d].width,4*a[4*d+4*l].width,4*a[4*d+8*l].width],v=[16*a[4*d].width-16*s,8*a[4*d+4*l].width-8*s,4*a[4*d+8*l].width-4*s];for(e=0;e<j.stage_classifier.length;e++){var w=j.stage_classifier[e].orig_feature,x=j.stage_classifier[e].feature=new Array(j.stage_classifier[e].count);for(f=0;f<j.stage_classifier[e].count;f++)for(x[f]={size:w[f].size,px:new Array(w[f].size),pz:new Array(w[f].size),nx:new Array(w[f].size),nz:new Array(w[f].size)},i=0;i<w[f].size;i++)x[f].px[i]=4*w[f].px[i]+w[f].py[i]*u[w[f].pz[i]],x[f].pz[i]=w[f].pz[i],x[f].nx[i]=4*w[f].nx[i]+w[f].ny[i]*u[w[f].nz[i]],x[f].nz[i]=w[f].nz[i]}for(i=0;i<4;i++){var y=[a[4*d].data,a[4*d+4*l].data,a[4*d+8*l+i].data],z=[8*p[i]+q[i]*a[4*d].width*8,4*p[i]+q[i]*a[4*d+4*l].width*4,0];for(h=0;h<t;h++){for(g=0;g<s;g++){var A=0,B=!0;for(e=0;e<j.stage_classifier.length;e++){A=0;var C=j.stage_classifier[e].alpha,x=j.stage_classifier[e].feature;for(f=0;f<j.stage_classifier[e].count;f++){var D,E,F=x[f],G=y[F.pz[0]][z[F.pz[0]]+F.px[0]],H=y[F.nz[0]][z[F.nz[0]]+F.nx[0]];if(G<=H)A+=C[2*f];else{var I,J=!0;for(I=0;I<F.size;I++){if(F.pz[I]>=0&&(D=y[F.pz[I]][z[F.pz[I]]+F.px[I]],D<G)){if(D<=H){J=!1;break}G=D}if(F.nz[I]>=0&&(E=y[F.nz[I]][z[F.nz[I]]+F.nx[I]],E>H)){if(G<=E){J=!1;break}H=E}}A+=J?C[2*f+1]:C[2*f]}}if(A<j.stage_classifier[e].threshold){B=!1;break}}B&&r.push({x:(4*g+2*p[i])*n,y:(4*h+2*q[i])*o,width:j.width*n,height:j.height*o,neighbor:1,confidence:A}),z[0]+=16,z[1]+=8,z[2]+=4}z[0]+=v[0],z[1]+=v[1],z[2]+=v[2]}}n*=k,o*=k}return r}function i(a){var b,c,d=this.shared.min_neighbors,e=this.shared.cascade;this.shared.interval,this.shared.scale,this.shared.next,this.shared.scale_upto;for(b=0;b<e.stage_classifier.length;b++)e.stage_classifier[b].feature=e.stage_classifier[b].orig_feature;if(a=a[0],d>0){var f=g.array_group(a,function(a,b){var c=Math.floor(.25*a.width+.5);return b.x<=a.x+c&&b.x>=a.x-c&&b.y<=a.y+c&&b.y>=a.y-c&&b.width<=Math.floor(1.5*a.width+.5)&&Math.floor(1.5*b.width+.5)>=a.width}),h=f.cat,i=f.index,j=new Array(h+1);for(b=0;b<j.length;b++)j[b]={neighbors:0,x:0,y:0,width:0,height:0,confidence:0};for(b=0;b<a.length;b++){var k=a[b],l=i[b];0==j[l].neighbors&&(j[l].confidence=k.confidence),++j[l].neighbors,j[l].x+=k.x,j[l].y+=k.y,j[l].width+=k.width,j[l].height+=k.height,j[l].confidence=Math.max(j[l].confidence,k.confidence)}var m=[];for(b=0;b<h;b++){var n=j[b].neighbors;n>=d&&m.push({x:(2*j[b].x+n)/(2*n),y:(2*j[b].y+n)/(2*n),width:(2*j[b].width+n)/(2*n),height:(2*j[b].height+n)/(2*n),neighbors:j[b].neighbors,confidence:j[b].confidence})}var o=[];for(b=0;b<m.length;b++){var k=m[b],p=!0;for(c=0;c<m.length;c++){var q=m[c],r=Math.floor(.25*q.width+.5);if(b!=c&&k.x>=q.x-r&&k.y>=q.y-r&&k.x+k.width<=q.x+q.width+r&&k.y+k.height<=q.y+q.height+r&&(q.neighbors>Math.max(3,k.neighbors)||k.neighbors<3)){p=!1;break}}p&&o.push(k)}return o}return a}if(void 0!==this.shared){var j=b(arguments,["canvas","cascade","interval","min_neighbors"]);this.shared.canvas=j.canvas,this.shared.interval=j.interval,this.shared.min_neighbors=j.min_neighbors,this.shared.cascade=j.cascade,this.shared.scale=Math.pow(2,1/(j.interval+1)),this.shared.next=j.interval+1,this.shared.scale_upto=Math.floor(Math.log(Math.min(j.canvas.width/j.cascade.width,j.canvas.height/j.cascade.height))/Math.log(this.shared.scale));var k;for(k=0;k<this.shared.cascade.stage_classifier.length;k++)this.shared.cascade.stage_classifier[k].orig_feature=this.shared.cascade.stage_classifier[k].feature}return{pre:f,core:h,post:i}})},h=window.onmessage||function(){};onmessage=function(a){var b;try{if(b="string"==typeof a.data?JSON.parse(a.data):a.data,"jquery.facedetection"===b.type){var c={shared:b.shared},d=f.core[b.name].apply(c,[b.input,b.id,b.worker]);try{postMessage(d)}catch(a){postMessage(JSON.stringify(d))}}else{var e=Array.prototype.slice.call(arguments);h.apply(window,e)}}catch(a){var e=Array.prototype.slice.call(arguments);h.apply(window,e)}},a.fn.faceDetection=function(b){"use strict";var d,e={interval:4,minNeighbors:1,grayscale:!0,confidence:null,async:!1,complete:function(){},error:function(){}};return a.isFunction(b)?e.complete=b:a.extend(e,b),this.each(function(){function b(){var a,b;if(d=(new Date).getTime(),h.is("img"))a=new Image,a.src=h.attr("src"),a.crossOrigin=h.attr("crossorigin"),b=g.pre(a);else if(h.is("video")||h.is("canvas")){var i,j;a=h[0],i=document.createElement("canvas"),i.setAttribute("width",a.videoWidth||a.width),i.setAttribute("height",a.videoHeight||a.height),j=i.getContext("2d"),j.drawImage(a,0,0),b=g.pre(i)}e.grayscale&&(b=g.grayscale(b));try{e.async&&window.Worker?g.detect_objects({canvas:b,cascade:c,interval:e.interval,min_neighbors:e.minNeighbors,worker:1,async:!0})(f):f(g.detect_objects({canvas:b,cascade:c,interval:e.interval,min_neighbors:e.minNeighbors}))}catch(a){e.error.apply(h,[2,a.message]),e.complete.apply(h,[!1])}}function f(a){for(var b=a.length,c=[],f=0;f<b;++f)null!==e.confidence&&a[f].confidence<=e.confidence||(a[f].positionX=j.left+a[f].x,a[f].positionY=j.top+a[f].y,a[f].offsetX=i.left+a[f].x,a[f].offsetY=i.top+a[f].y,a[f].scaleX=k,a[f].scaleY=l,c.push(a[f]));c.time=(new Date).getTime()-d,e.complete.apply(h,[c])}var h=a(this),i=h.offset(),j=h.position(),k=h.width()/(this.naturalWidth||this.videoWidth)||1,l=h.height()/(this.naturalHeight||this.videoHeight)||1;return h.is("img, video, canvas")?b():(e.error.apply(h,[1,"Face detection is possible on images, videos and canvas only."]),void e.complete.apply(h,[[]]))})}}("function"==typeof jQuery?jQuery:"function"==typeof Zepto?Zepto:{fn:{}});
